...
router.get('/users', adminAuth, async (req, res) => {
  try {
    ...
    const [users, total] = await Promise.all([
      prisma.user.findMany({ skip, take: limit, orderBy: { createdAt: 'desc' }, select: { id: true, phone: true, country: true, status: true, walletAddress: true, createdAt: true, _count: { select: { milestones: true } } } }),
      prisma.user.count()
    ]);

    res.json({ users: users.map(u => ({ id: u.id, phone: u.phone, country: u.country, status: u.status, walletAddress: u.walletAddress, milestones: u._count.milestones, createdAt: u.createdAt })), pagination: { page, limit, total, pages: Math.ceil(total / limit) } });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
router.get('/redemptions', adminAuth, async (req, res) => {
  try {
    ...
    const [redemptions, total] = await Promise.all([
      prisma.redemption.findMany({ skip, take: limit, orderBy: { createdAt: 'desc' }, include: { user: { select: { phone: true } }, partners: { select: { name: true } }, items: { include: { product: { select: { name: true } } } } } }),
      prisma.redemption.count()
    ]);

    res.json({ redemptions: redemptions.map(r => ({ id: r.id, userPhone: r.user.phone, partnerName: r.partners?.name || 'Unknown', productName: r.items[0]?.product?.name || 'Unknown', tokensSpent: r.totalTokens, voucherCode: r.items[0]?.voucherCode || null, status: r.status, createdAt: r.createdAt })), pagination: { page, limit, total, pages: Math.ceil(total / limit) } });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
